...
function getPreferenceFromProfile(a, b) {
  var c = CC["@mozilla.org/preferences-service;1"].getService(CI.nsIPrefBranch), d = fxdriver.logging.getLogger("fxdriver.badCertListener");
  if (!c.prefHasUserValue(a)) {
    return goog.log.info(d, a + " not set; defaulting to " + b), b;
  }
  c = c.getBoolPref(a);
  goog.log.info(d, "Found preference for " + a + ": " + c);
  return c;
}
...
var WdCertOverrideService = function() {
  shouldAssumeUntrustedIssuer() ? this.default_bits = this.ERROR_UNTRUSTED : this.default_bits = 0;
  var a = shouldAcceptUntrustedCerts();
  goog.log.info(WdCertOverrideService.LOG_, "Accept untrusted certificates: " + a);
  a && CC["@mozilla.org/preferences-service;1"].getService(CI.nsIPrefBranch).setIntPref("test.currentTimeOffsetSeconds", 11491200);
  this.origListener_ = Components.classesByID["{67ba681d-5485-4fff-952c-2ee337ffdcd6}"].getService().QueryInterface(CI.nsICertOverrideService);
};
WdCertOverrideService.LOG_ = fxdriver.logging.getLogger("fxdriver.WdCertOverrideService");
WdCertOverrideService.prototype = {ERROR_UNTRUSTED:1, ERROR_MISMATCH:2, ERROR_TIME:4};
WdCertOverrideService.prototype.certificateExpiredBit_ = function(a, b) {
  return 0 != (b & a.CERT_EXPIRED) ? (goog.log.info(WdCertOverrideService.LOG_, "Certificate expired."), this.ERROR_TIME) : 0;
};
WdCertOverrideService.prototype.certificateIssuerUntrusted_ = function(a, b) {
  return 0 != (b & a.ISSUER_UNKNOWN) || 0 != (b & a.ISSUER_NOT_TRUSTED) || 0 != (b & a.CERT_NOT_TRUSTED) || 0 != (b & a.INVALID_CA) ? (goog.log.info(WdCertOverrideService.LOG_, "Certificate issuer unknown."), goog.log.info(WdCertOverrideService.LOG_, "Unknown: " + (a.ISSUER_UNKNOWN & b)), goog.log.info(WdCertOverrideService.LOG_, "Issuer not trusted: " + (a.ISSUER_NOT_TRUSTED & b)), goog.log.info(WdCertOverrideService.LOG_, "Cert not trusted: " + (a.CERT_NOT_TRUSTED & b)), goog.log.info(WdCertOverrideService.LOG_, 
  "Invalid CA: " + (a.INVALID_CA & b)), this.ERROR_UNTRUSTED) : 0;
};
WdCertOverrideService.prototype.certificateHostnameMismatch_ = function(a, b) {
  var c = new RegExp("^" + a.commonName.replace("*", "[\\w|-]+") + "$", "i");
  return null === b.match(c) ? (goog.log.info(WdCertOverrideService.LOG_, "Host name mismatch: cert: " + a.commonName + " get: " + b), this.ERROR_MISMATCH) : 0;
};
WdCertOverrideService.prototype.fillNeededBits = function(a, b) {
  var c = a.verifyForUsage(a.CERT_USAGE_SSLClient), d = this.default_bits;
  goog.log.info(WdCertOverrideService.LOG_, "Certificate verification results: " + c);
  d |= this.certificateExpiredBit_(a, c);
  d |= this.certificateHostnameMismatch_(a, b);
  0 == d && (goog.log.info(WdCertOverrideService.LOG_, "Checking issuer since certificate has not expired or has a host name mismatch."), d |= this.certificateIssuerUntrusted_(a, c));
  goog.log.info(WdCertOverrideService.LOG_, "return_bits now: " + d);
  return d;
};
WdCertOverrideService.prototype.hasMatchingOverride = function(a, b, c, d, e) {
  var f = !1;
  !0 === shouldAcceptUntrustedCerts() ? (goog.log.info(WdCertOverrideService.LOG_, "Allowing certificate from site: " + a + ":" + b), f = !0, e.value = !1, bot.userAgent.isProductVersion(19) ? (goog.log.info(WdCertOverrideService.LOG_, "Setting all Override Bits"), d.value = this.ERROR_UNTRUSTED | this.ERROR_MISMATCH | this.ERROR_TIME) : d.value = this.fillNeededBits(c, a), goog.log.info(WdCertOverrideService.LOG_, "Override Bits: " + d.value)) : f = this.origListener_.hasMatchingOverride(a, b, c, 
  d, e);
  return f;
};
WdCertOverrideService.prototype.clearValidityOverride = function(a, b) {
  this.origListener_.clearValidityOverride(a, b);
};
WdCertOverrideService.prototype.getAllOverrideHostsWithPorts = function(a, b) {
  this.origListener_.getAllOverrideHostsWithPorts(a, b);
};
WdCertOverrideService.prototype.getValidityOverride = function(a, b, c, d, e, f) {
  return this.origListener_.getValidityOverride(a, b, c, d, e, f);
};
WdCertOverrideService.prototype.isCertUsedForOverrides = function(a, b, c) {
  return this.origListener_.isCertUsedForOverrides(a, b, c);
};
WdCertOverrideService.prototype.rememberValidityOverride = function(a, b, c, d, e) {
  this.origListener_.rememberValidityOverride(a, b, c, d, e);
};
WdCertOverrideService.prototype.QueryInterface = function(a) {
  if (a.equals(CI.nsICertOverrideService) || a.equals(CI.nsIInterfaceRequestor) || a.equals(CI.nsISupports)) {
    return this;
  }
  throw CR.NS_ERROR_NO_INTERFACE;
};
var CERTOVERRIDE_CONTRACT_ID = "@mozilla.org/security/certoverride;1", DUMMY_CERTOVERRIDE_SERVICE_CLASS_ID = Components.ID("{c8fffaba-9b7a-41aa-872d-7e7366c16715}"), service = void 0, WDCertOverrideFactory = {createInstance:function(a, b) {
  if (null != a) {
    throw CR.NS_ERROR_NO_AGGREGATION;
  }
  void 0 == service && (service = new WdCertOverrideService);
  return service;
}};
function WDBadCertListenerModule() {
  this.firstTime_ = !0;
}
WDBadCertListenerModule.LOG_ = fxdriver.logging.getLogger("fxdriver.WDBadCertListenerModule");
WDBadCertListenerModule.prototype.registerSelf = function(a, b, c, d) {
  if (this.firstTime_) {
    throw this.firstTime_ = !1, CR.NS_ERROR_FACTORY_REGISTER_AGAIN;
  }
  goog.log.info(WDBadCertListenerModule.LOG_, "Registering Override Certificate service.");
  a = a.QueryInterface(CI.nsIComponentRegistrar);
  a.registerFactoryLocation(DUMMY_CERTOVERRIDE_SERVICE_CLASS_ID, "badCertListener", CERTOVERRIDE_CONTRACT_ID, b, c, d);
};
WDBadCertListenerModule.prototype.unregisterSelf = function(a, b, c) {
  goog.log.info(WDBadCertListenerModule.LOG_, "Un-registering Override Certificate service.");
  a = a.QueryInterface(CI.nsIComponentRegistrar);
  a.unregisterFactoryLocation(DUMMY_CERTOVERRIDE_SERVICE_CLASS_ID, b);
};
var FACTORY;
bot.userAgent.isProductVersion("10") || (FACTORY = {createInstance:function(a, b) {
  if (null != a) {
    throw CR.NS_ERROR_NO_AGGREGATION;
  }
  if (void 0 != service) {
  ...
  var c = new WdCertOverrideService, d = CC["@mozilla.org/thread-manager;1"].getService().mainThread;
  return service = CC["@mozilla.org/xpcomproxy;1"].getService(CI.nsIProxyObjectManager).getProxyForObject(d, CI.nsICertOverrideService, c, 5);
}}, WdCertOverrideService.prototype._xpcom_factory = FACTORY);
WDBadCertListenerModule.prototype.getClassObject = function(a, b, c) {
  if (!c.equals(CI.nsIFactory)) {
    throw CR.NS_ERROR_NOT_IMPLEMENTED;
  }
  if (b.equals(DUMMY_CERTOVERRIDE_SERVICE_CLASS_ID)) {
    return bot.userAgent.isProductVersion("10") ? WDCertOverrideFactory : FACTORY;
  }
  throw CR.NS_ERROR_NO_INTERFACE;
};
WDBadCertListenerModule.prototype.canUnload = function(a) {
  return!0;
};
...
XPCOMUtils.generateNSGetFactory && (NSGetFactory = XPCOMUtils.generateNSGetFactory([WdCertOverrideService]));
