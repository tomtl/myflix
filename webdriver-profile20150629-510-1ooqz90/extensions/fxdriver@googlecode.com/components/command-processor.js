...
fxdriver.proxy.setProxyPreference_ = function(a, b, c) {
  c && (c = c.split(":"), a.setCharPref("network.proxy." + b, c[0]), 1 < c.length && a.setIntPref("network.proxy." + b + "_port", parseInt(c[1])));
};
...
fxdriver.proxy.pacConfig_ = function(a, b) {
  ...
  var c = fxdriver.moz.getService("@mozilla.org/docshell/urifixup;1", "nsIURIFixup").createFixupURI(b.proxyAutoconfigUrl, 0).spec;
  a.setCharPref("network.proxy.autoconfig_url", c);
};
...
wdSessionStoreService.prototype.createSession = function(a, b, c, d) {
  var e = Components.classes["@mozilla.org/uuid-generator;1"].getService(Components.interfaces.nsIUUIDGenerator).generateUUID().toString(), e = e.substring(1, e.length - 1), f = Components.classes["@googlecode.com/webdriver/wdsession;1"].createInstance(Components.interfaces.nsISupports);
  f.wrappedJSObject.setId(e);
...
wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING = {webStorageEnabled:"dom.storage.enabled", applicationCacheEnabled:"browser.cache.offline.enable", databaseEnabled:"dom.indexedDB.enabled", locationContextEnabled:"geo.enabled", browserConnectionEnabled:"dom.network.enabled", acceptSslCerts:"webdriver_accept_untrusted_certs", nativeEvents:"webdriver_enable_native_events", pageLoadingStrategy:"webdriver.load.strategy", pageLoadStrategy:"webdriver.load.strategy"};
wdSessionStoreService.prototype.configure_ = function(a, b, c, d) {
  ...
  c && (goog.object.forEach(c, function(b, c) {
    if (goog.isBoolean(b) && c in wdSessionStoreService.READ_ONLY_CAPABILITIES_ && b != wdSessionStoreService.READ_ONLY_CAPABILITIES_[c]) {
      var d = "Required capability " + c + " cannot be set to " + b;
      goog.log.info(wdSessionStoreService.LOG_, d);
      a.sendError(new WebDriverError(bot.ErrorCode.SESSION_NOT_CREATED, d));
...
wdSessionStoreService.prototype.configureCapabilities_ = function(a, b) {
  ...
  goog.object.forEach(a, function(a, e) {
    if (e in wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING) {
      var f = wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING[e];
      goog.log.info(wdSessionStoreService.LOG_, "Setting capability " + e + " (" + f + ") to " + a);
      goog.isBoolean(a) ? c.setBoolPref(f, a) : c.setCharPref(f, a);
      "nativeEvents" == e && (b.enableNativeEvents = a);
    }
  });
  b.enableNativeEvents && c.setCharPref("layout.css.devPixelsPerPx", "1.0");
};
...
DelayedCommand.prototype.shouldDelayExecutionForPendingRequest_ = function() {
  ...
  if (this.loadGroup_.isPending()) {
    for (var a = !1, b = 0, c = this.loadGroup_.requests;c.hasMoreElements();) {
      var d = null, e = c.getNext();
      try {
        d = e.QueryInterface(Components.interfaces.nsIRequest);
      } catch (f) {
        goog.log.info(DelayedCommand.LOG_, "Ignoring non-nsIRequest: " + e);
        continue;
      }
      e = !1;
      try {
...
DelayedCommand.prototype.executeInternal_ = function() {
  ...
  if ("about:blank" != this.driver_.window.location || this.onBlank_) {
    try {
      ...
      var b = (new Date).getTime() + this.response_.session.getImplicitWait(), c = this.command_.name, d = this.driver_[c] || WebElement[c], e = this.command_.parameters, f = goog.bind(d, this.driver_, this.response_, e), g = goog.bind(this.checkPreconditions_, this, d.preconditions, this.response_, e), h = function() {
        try {
          g(), f();
        } catch (d) {
          (new Date).getTime() < b ? DelayedCommand.execTimer.setTimeout(h, 100) : (d.isWebDriverError || goog.log.error(DelayedCommand.LOG_, "Exception caught by driver: " + c + "(" + e + ")", d), a.sendError(d));
        }
      };
      h();
    } catch (k) {
      k.isWebDriverError || goog.log.error(DelayedCommand.LOG_, "Exception caught by driver: " + this.command_.name + "(" + this.command_.parameters + ")", k), this.response_.sendError(k);
    }
  } else {
...
nsCommandProcessor.prototype.execute = function(a, b) {
  ...
  d = new fxdriver.CommandResponse(c, b);
  if ("newSession" == c.name || "quit" == c.name || "getStatus" == c.name || "getWindowHandles" == c.name) {
    ...
    if (g) {
      ...
      var k = d.session.getChromeWindow();
      if (g = k.fxdriver) {
        if (k.getBrowser().contentWindow) {
          if (g.modalOpen && "getAlertText" != c.name && "setAlertValue" != c.name && "acceptAlert" != c.name && "dismissAlert" != c.name) {
            switch(c = g.modalOpen, fxdriver.modals.getUnexpectedAlertBehaviour()) {
              case "accept":
                fxdriver.modals.closeUnhandledAlert(d, g, !0);
                break;
              case "ignore":
                d.sendError(new WebDriverError(bot.ErrorCode.UNEXPECTED_ALERT_OPEN, "Modal dialog present", {alert:{text:c}}));
                break;
              default:
                fxdriver.modals.closeUnhandledAlert(d, g, !1);
            }
          } else {
            "function" != typeof g[c.name] && "function" != typeof WebElement[c.name] ? (d.sendError(new WebDriverError(bot.ErrorCode.UNKNOWN_COMMAND, "Unrecognised command: " + c.name)), goog.log.error(nsCommandProcessor.LOG_, "Unknown command: " + c.name)) : ("get" != c.name && "refresh" != c.name || d.session.setWaitForPageLoad(!1), d.startCommand(k), (new DelayedCommand(g, c, d)).execute(0));
          }
        } else {
...
nsCommandProcessor.prototype.getLog = function(a, b) {
  var c = fxdriver.logging.getLog(b.type);
  goog.array.forEach(c, function(a) {
...
nsCommandProcessor.prototype.newSession = function(a, b) {
  ...
  if (d) {
    var e = Components.classes["@googlecode.com/webdriver/wdsessionstoreservice;1"].getService(Components.interfaces.nsISupports), f = b.desiredCapabilities, d = e.wrappedJSObject.createSession(a, f, b.requiredCapabilities, d), d = d.wrappedJSObject;
    d.setChromeWindow(c);
    "elementScrollBehavior" in f && (d.elementScrollBehavior = f.elementScrollBehavior);
    a.session = d;
    a.sessionId = d.getId();
    goog.log.info(nsCommandProcessor.LOG_, "Created a new session with id: " + d.getId());
...
nsCommandProcessor.prototype.getSessionCapabilities = function(a) {
  ...
  a.value = {cssSelectorsEnabled:!0, browserName:"firefox", handlesAlerts:!0, javascriptEnabled:!0, nativeEvents:Utils.useNativeEvents(), platform:"WINNT" == c.OS ? "WINDOWS" : c.OS, rotatable:!1, takesScreenshot:!0, version:b.version};
